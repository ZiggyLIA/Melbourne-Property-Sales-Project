...
# This function is to search if there are elements in queried_properties (include Suburb, Address, Postcode) to match the corresponding data in  main_data
# If there is a matching data, then print the remaining information for that property
def provide_realestate_info(main_data, queried_properties):
    # replace 'pass' with your code
    # Convert json string into a list of dictionaries
    data_list = json.loads(queried_properties)
    result = []
    # Loop and extract the values from the data_list
    for d in data_list:
        suburb = d['Suburb']
        address = d['Address']
        postcode = d['Postcode']
        # Construct the format of the result
        print_out_data = {'Address': None,
                          'Bedrooms': None,
                          'Landsize': None,
                          'Postcode': None,
                          'Price': None,
                          'Suburb': None}
        # Count how many much data found in the data_cleaned
        # Add the match data to the match_data list
        count = 0
        match_data = []
        for key, value in main_data.items():
            if (suburb == value['Suburb'] and address in value['Address']
                    and str(postcode) == value['Postcode']):
                match_data.append(value)
                count += 1
        # Add the match data to the match_data list and\
        # plug the corresponding data to the constructed format
        if count == 1:
            print_out_data['Address'] = match_data[0]['Address']
            print_out_data['Bedrooms'] = int(match_data[0]['Bedrooms'])
            print_out_data['Landsize'] = int(match_data[0]['Landsize'])
            print_out_data['Postcode'] = int(match_data[0]['Postcode'])
            print_out_data['Price'] = int(match_data[0]['Price'])
            print_out_data['Suburb'] = match_data[0]['Suburb']
            result.append(print_out_data)
        # More than 1 or less than 1 of match data will not be printed out
        elif count != 1:
            continue
    return result
